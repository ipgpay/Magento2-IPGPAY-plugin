...
class Go extends Action
{
    ...
    private function getPaymentFormUrl()
    {
        return preg_replace('#/payment/form/post#i', '',
            $this->_scopeConfig->getValue('payment/ipgpay_gateway/payment_form_url',ScopeInterface::SCOPE_STORE) . '/payment/form/post');
    }
    ...
    private function getOrderItemsParameters(Order $order)
    {
        ...
        foreach($orderItems as $item)
        {
            ...
            $items[] = $this->getItemArray(
                ...
                FALSE,
                $this->_scopeConfig->getValue('payment/ipgpay_gateway/merchant_rebilling',ScopeInterface::SCOPE_STORE) == '1' ? '1' : '0'
            );
    ...
    {
        $this->_url->getUrl('ipgpay/gateway/success');
        return array(
            'client_id' => $this->_scopeConfig->getValue('payment/ipgpay_gateway/account_id',ScopeInterface::SCOPE_STORE),
            'return_url' => $this->_url->getUrl('ipgpay/gateway/cancel'), // return to the shop before compeleting purchase
            'approval_url' => $this->_url->getUrl('ipgpay/gateway/success'),
            'decline_url' => $this->_url->getUrl('ipgpay/gateway/cancel'),
            'test_transaction' => $this->_scopeConfig->getValue('payment/ipgpay_gateway/test_mode',ScopeInterface::SCOPE_STORE) == '1' ? '1' : '0',
            'order_reference' => $order->getIncrementId(),
            'order_currency' => $order->getOrderCurrencyCode(),
            'form_id' => $this->_scopeConfig->getValue('payment/ipgpay_gateway/payment_form_id',ScopeInterface::SCOPE_STORE),
            'merchant_name' => $this->_scopeConfig->getValue('general/store_information/name',ScopeInterface::SCOPE_STORE),
            'create_customer' => $this->_scopeConfig->getValue('payment/ipgpay_gateway/create_customers',ScopeInterface::SCOPE_STORE) == '1' ? '1' : '0'
        );
    }
    ...
    private function mergeFormParameters($customerParameters, $orderItemsParameters, $gatewayParameters)
    {
        ...
        $signatureLifetime = $this->_scopeConfig->getValue('payment/ipgpay_gateway/request_expiry',ScopeInterface::SCOPE_STORE);
        ...
        $fields['PS_SIGNATURE'] = $this->createSignature($fields);
    ...
    {
        $secret = $this->_scopeConfig->getValue('payment/ipgpay_gateway/secret_key',ScopeInterface::SCOPE_STORE);
        $sigstring = $secret;
